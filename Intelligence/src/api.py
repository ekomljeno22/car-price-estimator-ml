...
app = FastAPI(title="Used Cars ML Microservice", version="1.3.0")

MODEL, MODEL_TYPE, TE_MAPS, RARE_MAPS, SCALER, OHE, ORD_ENC, META = [None] * 8

CHART_NAMES = {
    "01_price_distribution.png":   "Distribucija cijena",
    "02_outlier_boxplots.png":     "Boxplot outlieri",
    "03_mileage_vs_price.png":     "Kilometraža vs Cijena",
    "04_correlation_heatmap.png":  "Korelacijska heatmapa",
    "05_brand_median_price.png":   "Medijanska cijena po marki",
    "06_price_by_year.png":        "Cijena po godini",
    "07_fuel_type_distribution.png": "Distribucija goriva",
    "08_accident_vs_price.png":    "Nezgode vs Cijena",
    "error_distribution.png":     "Distribucija grešaka modela",
    "feature_importance.png":     "Važnost značajki",
    "prediction_vs_actual.png":   "Predikcija vs Stvarna cijena",
}
...
def startup_event():
    ...
    try:
        ...
        if hasattr(MODEL, "feature_names_in_"):
            print(f"📋 [MODEL] Model očekuje: {list(MODEL.feature_names_in_)}")
        else:
            print(f"📋 [MODEL] Model nema zapisana imena stupaca, broj: {getattr(MODEL, 'n_features_in_', 'nepoznato')}")
...
def predict(req: PredictRequest):
    ...
    try:
        ...
        num_vals = np.array([[req.model_year, model_age, req.milage, miles_per_year, req.hp, req.liters]])
...
def get_stats():
    ...
    if not os.path.exists(metrics_path):
        return {
            ...
            "mape": 0.0, "train_mape": 0.0, "train_r2": 0.0,
...
def list_charts():
    """
    Vraća JSON popis svih .png grafova iz mape results/.
    Svaki unos sadrži:
      - filename  : ime datoteke
      - label     : čitljivo ime grafa
      - url       : relativni URL za preuzimanje slike (/charts/{filename})
    """
    results_dir = "results"
    ...
    for filename in sorted(os.listdir(results_dir)):
        if filename.lower().endswith(".png"):
            charts.append({
                "filename": filename,
                "label":    CHART_NAMES.get(filename, filename.replace("_", " ").replace(".png", "").title()),
...
def get_chart(filename: str):
    """
    Vraća PNG sliku iz results/ mape.
    Primjer: GET /charts/03_mileage_vs_price.png
    """
    ...
    if not os.path.isfile(path):
        raise HTTPException(status_code=404, detail=f"Graf '{filename}' nije pronađen.")
